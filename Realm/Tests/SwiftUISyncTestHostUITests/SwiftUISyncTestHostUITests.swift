...
extension SwiftUISyncTestHostUITests {
    ...
    func testAsyncOpenWithDeferRealmConfiguration() throws {
        let email = "realm_tests_do_autoverify\(randomString(7))@\(randomString(7)).com"
        let user = try createUsers(email: email, password: password, n: 20)
        ...
        let enumerator = FileManager.default.enumerator(at: appClientDataRoot, includingPropertiesForKeys: [.nameKey, .isDirectoryKey])
        var counter = 0
